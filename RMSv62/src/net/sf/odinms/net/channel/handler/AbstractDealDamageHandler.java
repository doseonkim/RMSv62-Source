    ...
    protected synchronized void applyAttack(AttackInfo attack, MapleCharacter player, int maxDamagePerMonster, int attackCount) {
        ...
        for (Pair<Integer, List<Integer>> oned : attack.allDamage) {
            ...
            if (monster != null) {
                ...
                if ((((totDamageToOneMonster > 100000000) && (player.getReborns() < 50)) || (((totDamageToOneMonster > 150000000) && (player.getReborns() < 100))))) {
				AutobanManager.getInstance().autoban(player.getClient(), "Damage hacking ("+ (totDamageToOneMonster +") with "+player.getReborns()+" reborn(s)."));
