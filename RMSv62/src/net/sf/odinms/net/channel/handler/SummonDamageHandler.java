...
public class SummonDamageHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        for (SummonAttackEntry attackEntry : allDamage) {
            ...
            if (target != null) {
                if ((((damage > 50000000) && (player.getReborns() < 50)) || (((damage > 100000000) && (player.getReborns() < 100))))) {
				AutobanManager.getInstance().autoban(player.getClient(), "Damage hacking ("+ (damage +") with "+player.getReborns()+" reborn(s)."));
