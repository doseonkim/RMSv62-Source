...
function action(mode, type, selection) {
	...
	if (cm.getJob().getId() % 10 == 2) {
		...
		if ((secondJob && cm.getLevel() < 70) || (!secondJob && cm.getLevel() < 120)) {
		    ...
		    if (status == 0)
			    cm.sendYesNo("#eGreat job getting to level "+cm.getLevel()+". Would you like to become a #b"+cm.getJobById(newJob)+"#k ?");
		    else if (status == 1) {
			    cm.sendOk("#eCongratulations, you are now a #b"+cm.getJobById(newJob)+"#k.");
		...
		if (status == 0) {
			...
			if (possibleJobs.length == 0) {
				cm.sendOk("#eYour level is too low to advance.");
				cm.dispose();
			} else {
			...
			cm.sendYesNo("#eAre you sure you want to job advance?");
			job = selection;
		} else if (status == 2) {
			cm.sendOk("#eCongratulations on your job advancement.");
